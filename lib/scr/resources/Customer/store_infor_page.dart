...
class DetailStore extends StatefulWidget {
  ...
  DetailStore(this.index,this.name,this.image,this.city,this.district,this.address,this.description,this.start,this.end,this.uid);
  @override
...
class _DetailStoreState extends State<DetailStore> {
  ...
  void initState() {
    ...
    query = FirebaseDatabase.instance.reference().child("Stores").child(widget.uid).child("Comment").child("comment");
    DatabaseReference reference = FirebaseDatabase.instance.reference().child("Stores").child(widget.uid);
    DatabaseReference reference2 = FirebaseDatabase.instance.reference().child("Stores").child(widget.uid).child("Like");
    DatabaseReference referenceComment = FirebaseDatabase.instance.reference().child("Stores").child(widget.uid).child("Comment");
            ...
            flexibleSpace: Container(
              ...
              decoration: BoxDecoration(
                  ...
                  image: DecorationImage(
                      image:NetworkImage(widget.image??"https://scontent.fhan4-1.fna.fbcdn.net/v/t1.0-9/135634832_221471026121571_925480074581197249_o.jpg?_nc_cat=105&ccb=2&_nc_sid=b9115d&_nc_ohc=iZhweHX5XFwAX_qB48d&_nc_ht=scontent.fhan4-1.fna&oh=f4de4a1922e6e0570e7e136cfc62d2db&oe=601B77B2"), fit: BoxFit.cover)),
            ),
          ),
        ),
                      ...
                      InkWell(
                        onTap: (){
                          final action = CupertinoActionSheet(
                            title: Text(
                              "Chia sẻ", style: TextStyle(fontSize: 30),
                            ),
                            message: Text(
                            ...
                            actions: <Widget>[
                              CupertinoActionSheetAction(
                                ...
                                onPressed: () {
                                  FlutterShareMe().shareToFacebook(
                                    url: 'https://github.com/hungcb21/Flutter_APP', msg: widget.name
                                    ,);
                              ...
                              CupertinoActionSheetAction(
                                ...
                                onPressed: () async {
                                  var response = await FlutterShareMe().shareToSystem(msg: 'https://github.com/hungcb21/Flutter_APP');
                                  Navigator.of(context, rootNavigator: true).pop();
                                },
                              )
                            ],
  ...
  Widget createButton({icon, color, text}) {
    return ButtonTheme(
      ...
      child: FlatButton.icon(
        onPressed: (){Navigator.push(context, MaterialPageRoute(builder: (context)=>ChooseTime(widget.uid,widget.name,widget.address,widget.district,widget.city)));},
        icon: icon,
        label: Text(
          text,
  ...
  Future<bool> onLikeButtonTapped(bool isLiked) async{
    ...
    {
      like = like+1;
      var ref = FirebaseDatabase.instance.reference().child("Stores").child(widget.uid);
  ...
  void onCommentClick() {
    ...
    {
      commenta = commenta+1;
      var ref2 = FirebaseDatabase.instance.reference().child("Stores").child(widget.uid);
      var count ={"Count":commenta};
      var ref3 = FirebaseDatabase.instance.reference().child("Stores").child(widget.uid);
      var category ={"Image":avatar,"Comment":commentController.text,"Name":name};
